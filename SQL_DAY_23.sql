...
select
	--* from
	distinct a1.user_id as active_user
	from
amazon_transactions as a1 -- first purchase table
join amazon_transactions as a2 -- sencond pusrchase table
...
JOIN amazon_transactions AS a2
  ...
  AND a1.purchase_date < a2.purchase_date
  AND a2.purchase_date - a1.purchase_date > 7
WHERE a1.user_id NOT IN (
  SELECT DISTINCT b1.user_id
  FROM amazon_transactions AS b1
  JOIN amazon_transactions AS b2
    ON b1.user_id = b2.user_id
    AND b1.purchase_date < b2.purchase_date
    AND b2.purchase_date - b1.purchase_date <= 7
)
ORDER BY 1;
